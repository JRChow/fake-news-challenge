...
class Word2VecFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df, header='train'):
        ...
        print('Headline_unigram_array: ',Headline_unigram_array.shape)
        ...
        headlineVec = np.array(list(headlineVec))
        # print('headlineVec: ', headlineVec.shape)
        # print headlineVec
        ...
        outfilename_hvec= "feature_pkl/"+header+".headline.word2vec.pkl"
        with open(outfilename_hvec, "wb") as outfile:
            pickle.dump(headlineVec, outfile, -1)
        print('headline word2vec features of data set saved in %s' % outfilename_hvec)
        ...
        print('Body_unigram_array: ',Body_unigram_array.shape)
        #bodyVec = [0]
        bodyVec = map(lambda x: reduce(np.add, [model[y] for y in x if y in model], [0.]*300), Body_unigram_array)
        bodyVec = np.array(list(bodyVec))
        bodyVec = normalize(bodyVec)
        print('bodyVec ',bodyVec.shape)

        # bodyVecTrain = bodyVec[:n_train, :]
        outfilename_bvec= "feature_pkl/"+header+".body.word2vec.pkl"
        with open(outfilename_bvec, "wb") as outfile:
            pickle.dump(bodyVec, outfile, -1)
        print('body word2vec features of data set saved in %s' % outfilename_bvec)
        ...
        simVec = np.asarray(list(map(self.cosine_sim, headlineVec, bodyVec)))[:, np.newaxis]
        print('simVec.shape:', simVec.shape)

        # simVecTrain = simVec[:n_train]
        outfilename_simvec= "feature_pkl/"+header+".sim.word2vec.pkl"
        with open(outfilename_simvec, "wb") as outfile:
            pickle.dump(simVec, outfile, -1)
        print('word2vec sim. features of data set saved in %s' % outfilename_simvec)
    ...
    def read(self, header='train'):
        ...
        filename_hvec = "feature_pkl/%s.headline.word2vec.pkl" % header
        with open(filename_hvec, "rb") as infile:
            headlineVec = pickle.load(infile)

        filename_bvec = "feature_pkl/%s.body.word2vec.pkl" % header
        with open(filename_bvec, "rb") as infile:
            bodyVec = pickle.load(infile)

        filename_simvec = "feature_pkl/%s.sim.word2vec.pkl" % header
