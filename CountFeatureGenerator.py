...
class CountFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df, header='train'):
        ...
        for feat_name in feat_names:
            for gram in grams:
                ...
                df["ratio_of_unique_%s_%s" % (feat_name, gram)] = \
                    list(map(self.try_divide, df["count_of_unique_%s_%s"%(feat_name,gram)], df["count_of_%s_%s"%(feat_name,gram)]))
        ...
        for gram in grams:
            ...
            df["ratio_of_Headline_%s_in_articleBody" % gram] = \
                list(map(self.try_divide, df["count_of_Headline_%s_in_articleBody" % gram], df["count_of_Headline_%s" % gram]))
        ...
        outfilename_bcf = "feature_pkl/"+header+".basic.pkl"
        with open(outfilename_bcf, "wb") as outfile:
            pickle.dump(feat_names, outfile, -1)
            pickle.dump(xBasicCounts, outfile, -1)
        print('basic counting features for training saved in %s' % outfilename_bcf)
    ...
    def read(self, header='train'):
        ...
        filename_bcf = "feature_pkl/%s.basic.pkl" % header
